...
export default async (container: AwilixContainer, configuration: PluginOptions) => {
    ...
    for (let i = 0; i < settingsSchemas.length; i++) {
        ...
        if (correspondingDatabaseSetting === undefined) {
            ...
            if (!validation) {
                console.info("[medusa-plugin-settings](settings-initializer):", `value of setting with id ${settingSchema.id} haven't passed validation (${correspondingDatabaseSetting.value}).`);
                medusaPluginSettingsService.resetSetting(settingSchema.id);
    ...
    if (databaseUnusedSettings.length > 0) {
        console.info("[medusa-plugin-settings](settings-initializer):", `(${databaseUnusedSettings.length}) unused setting(s) are present on the database => will be deleted ?`, configuration.shouldDeleteUnusedSettings || false);
        if (configuration.shouldDeleteUnusedSettings === true) {
